...
def test(loaders, model):
    accuracy_list = []
    iter_val = [iter(loader) for loader in loaders] # list of iterable dloader
    for i in xrange(len(iter_val)):                 # for each dloader/task???
        iter_ = iter_val[i]                         # current iterable dloader
        start_test = True
        for j in xrange(len(loaders[i])):
            data = iter_.next()
            inputs, labels = data
